namespace Tempore.Tests.Tempore.Client.WorkforceMetricClient
{
    ...
    public partial class WorkforceMetricClientFacts
    {
        ...
        public class The_GetWorkforceMetricsAsync_Method : EnvironmentTestBase
        {
            public The_GetWorkforceMetricsAsync_Method(
            ...
            public static IEnumerable<object[]> InvalidRequest()
            {
                yield return new object[]
                             {
                                 new GetWorkforceMetricsRequest
                                 {
                                     Skip = -1,
                                     Take = 0,
            ...
            public async Task Throws_ApiException_With_Status401Unauthorized_StatusCode_When_Invalid_Credentials_Are_Used_Async()
            {
                ...
                var apiException = await Assert.ThrowsAsync<ApiException>(
                                       () => workforceMetricClientClient.GetWorkforceMetricsAsync(
                                           new GetWorkforceMetricsRequest()));
            ...
            [Trait(Traits.Category, Category.Integration)]
            public async Task
                Throws_ApiException_With_Status400BadRequest_StatusCode_Invalid_Data_For_The_Request_Async(
                    GetWorkforceMetricsRequest request)
            {
                ...
                var apiException =
                    await Assert.ThrowsAsync<ApiException>(
                        () => workforceMetricClient.GetWorkforceMetricsAsync(request));
            ...
            [Fact]
            [Trait(Traits.Category, Category.Integration)]
            public async Task Returns_The_Expected_Workforce_Metrics_For_Each_Employees_Async()
            {
                // Devices
                ...
                var workforceMetrics = await workforceMetricClient.GetWorkforceMetricsAsync(request);
                workforceMetrics.Count.Should().Be(EmployeesCount);
                workforceMetrics.Items.Count.Should().Be(EmployeesCount);

                var row = workforceMetrics.Items.ToList();
                for (var i = 0; i < row.Count - 1; i++)
                {
                    for (var j = i + 1; j < row.Count; j++)
                    {
                        row[i].Keys.Should().BeEquivalentTo(row[j].Keys);
                    }
                }
            }
        }
