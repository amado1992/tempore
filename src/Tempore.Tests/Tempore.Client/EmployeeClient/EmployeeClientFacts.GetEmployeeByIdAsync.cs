...
public partial class EmployeeClientFacts
{
    ...
    public class The_GetEmployeeByIdAsync_Method : EnvironmentTestBase
    {
        public The_GetEmployeeByIdAsync_Method(
        ...
        public async Task Throws_ApiException_With_Status401Unauthorized_StatusCode_When_Invalid_Credentials_Are_Used_In_GetEmployeeById_Async()
        {
            ...
            var apiException = await Assert.ThrowsAsync<ApiException>(() => employeeClient.GetEmployeeByIdAsync(Guid.NewGuid()));
        ...
        [Fact]
        [Trait(Traits.Category, Category.Integration)]
        public async Task Succeeds_When_The_Employee_Exist_Async()
        {
            ...
            var employeeDto = await employeeClient.GetEmployeeByIdAsync(employee.Id);
